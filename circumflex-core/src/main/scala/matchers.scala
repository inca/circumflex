...
object MatchingUtils {
  ...
  def extractMatches(regex: Regex, src: String, groupPrefix: String): Option[Map[String, String]] = {
  ...
  def extractMatches(pattern: String, src: String, separators:String): Option[Map[String, String]] = {
    var matches = Map[String, String]()
    def extract(patternBuffer: List[Char], srcBuffer: List[Char]): Boolean = patternBuffer match {
...
case class UriRegexMatcher(val uriRegex: Regex) extends RequestMatcher {
  def matchRequest(request: HttpServletRequest) = extractMatches(uriRegex, request.getRequestURI, "uri$")
}

case class UriPatternMatcher(val uriPattern: String) extends RequestMatcher {
  def matchRequest(request: HttpServletRequest) = extractMatches(uriPattern, request.getRequestURI, "/")
}
...
abstract class RequestHeadersMatcher[C](val criteria: (String, C)*) extends RequestMatcher {
  ...
  private def matchRequest(request: HttpServletRequest, params: Option[Map[String,String]], chain: List[Pair[String,C]]): Option[Map[String,String]] =
    params match {
      case None => None
...
case class HeadersRegexMatcher(override val criteria: (String,Regex)*) extends RequestHeadersMatcher[Regex] {
  def matchHeader(headerName: String, headerValue: String, crit: Regex) = extractMatches(crit, headerValue, headerName + "$")
}

case class HeadersPatternMatcher(override val criteria: (String,String)*) extends RequestHeadersMatcher[String] {
  def matchHeader(headerName: String, headerValue: String, crit: String) = extractMatches(crit, headerValue, ":,; /")
}