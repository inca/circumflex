...
object Country
  ...
  validation
      .notEmpty(_.code)
      .pattern(_.code, "^[a-z]{2}$".r.pattern, "syntax")
}
...
class City extends Record[String, City] {
  ...
  val country = "country_code".VARCHAR(2).NOT_NULL
      .REFERENCES(Country).ON_DELETE(CASCADE)
...
object City
    extends City
    with Table[String, City]
    with Cacheable[String, City] {
...
class Capital extends Record[String, Capital] {
  ...
  val country = "country_id".VARCHAR(2).NOT_NULL
      .REFERENCES(Country).ON_DELETE(CASCADE)

  val city = "city_id".TEXT.NOT_NULL
      .REFERENCES(City).ON_DELETE(CASCADE)
...
class Membership extends Record[(String, String), Membership] {
  ...
  val project = "project".VARCHAR(255).NOT_NULL
      .REFERENCES(Project).ON_DELETE(CASCADE)

  def name = project.field

  val developer = "developer".VARCHAR(255).NOT_NULL
      .REFERENCES(Developer).ON_DELETE(CASCADE)
...
object Membership
    extends Membership
    with Table[(String, String), Membership]
...
class IdentNoAuto
    extends Record[Long, IdentNoAuto]
    with IdentityGenerator[Long, IdentNoAuto] {
...
class IdentAuto
    extends Record[Long, IdentAuto]
    with IdentityGenerator[Long, IdentAuto] {
...
class SeqNoAuto
    extends Record[Long, SeqNoAuto]
    with SequenceGenerator[Long, SeqNoAuto] {
