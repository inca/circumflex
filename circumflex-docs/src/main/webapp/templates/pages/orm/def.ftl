...
val createdAt = dateColumn("created_at")
                .default("current_timestamp")'?html}</pre>
<p>The default values are useful in cases where the value must be generated by the database
  at the time the record is inserted. The most common case is shown in our example above:
  the database will put the timestamp at which the record is inserted into
  <code>created_at</code> column.</p>
<p><strong>Note:</strong> if the default value is specified explicitly, it will be used in
  all <code>INSERT</code> statements. This means that the corresponding values in records
  <strong>will be lost</strong> after the record is saved the first time. If you don't want
  this to occur, use the method <code>default</code> on a <em>field</em> inside the
  <code>Record</code>.</p>
